...
## Emptying variables already in SUF_Remote
...
## Anonymization: Forming grouping variable
...
Once the data record has been prepared, it can be saved. The export functions `write_spss()` and `write_stata()` are available for this purpose. The file paths must be adapted in each case.
```{r write, eval=FALSE}
